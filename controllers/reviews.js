...
exports.updateReview = asyncHandler(async (req, res, next) => {
  ...
  if (!review) {
    return next(new ErrorResponse(`No review found with id ${req.params.bootcampId}`, 404));
  }

  // Make sure review belongs to user or user is admin
  if (review.user.toString() !== req.user.id && req.user.role !== 'admin') {
    return next(new ErrorResponse(`User not allowed to update review`, 401));
  }
...
exports.deleteReview = asyncHandler(async (req, res, next) => {
  ...
  // Make sure review belongs to user or user is admin
  if (review.user.toString() !== req.user.id && req.user.role !== 'admin') {
    return next(new ErrorResponse(`User not allowed to delete this review`, 401));
  }
